$(function(){
            ...
            layer.confirm('确认修改吗?', {
                ...
                yes:function (index) {
                    if(reg.test(updateName)){
                        $.post('/departAdmin/update_list/',{"dier":dier,"update_name":updateName,"old_name": canshu,"depname":depname},function (data) {
                            if(data.result[0].result == 1){
                                layer.alert('修改成功');
                                $(updateLi).html(updateName);
                                $(updateLi).next().html(data.result[0].us_updateName);
                            }else{
                                layer.alert('修改失败，目录已存在',function (index) {
                                    layer.close(index);
        ...
        if((new Date().getTime() - date_now) >= 200){
            ...
            if(kg){
                ...
                var canshu = $(this).prev().val().trim().replace(/\s/g,'');
                    ...
                    switch (dier){
                        case "一级目录":
                            luyou = "/departAdmin/first_info/";
                            params = {'first_name':canshu,"depname":depname};
                            break;
                        case "二级目录":
                            luyou = "/departAdmin/second_info/";
                            params = {'first_name':oneLevelName, 'second_name':canshu,"depname":depname};
                            break;
                        case "三级目录":
                            luyou = "/departAdmin/third_info/";
                            params = {'first_name':oneLevelName, 'second_name':twoLevelName,'third_name':canshu,"depname":depname};
                            break;
                    }
                if(reg.test(canshu)){
                    layer.confirm('确认添加吗？',{
        ...
        if(!dierstr){
            ...
            switch (dier){
                case "一级目录":
                    ...
                    $.post('/departAdmin/second_list/',{"first_name":oneLevelName,"depname":depname},function (data) {
                        ...
                        for(var i = 0;i < secondList.length; i++){
                            ...
                            secondStr += "<li><a title='修改' class='glyphicon glyphicon-pencil' id='update'></a>&nbsp;";
                ...
                case "二级目录":
                    ...
                    $.post('/departAdmin/third_list/',{"second_name":twoLevelName,"depname":depname},function (data) {
                        ...
                        for(var j = 0; j <thirdList.length; j ++){
                            ...
                            thirdStr += "<li><a title='修改' class='glyphicon glyphicon-pencil' id='update'></a>&nbsp;";
    ...
    $(".Level_content").on('click','#delete',function (e) {
        ...
        layer.confirm('确认删除吗？',{
            ...
            btn: ["确定", "取消"],
            yes:function (index) {
                 $.post("/departAdmin/delete_list/",{"dier":dier,"delete_name" : deleteName,"depname":depname},function (data) {
                    if(data.result == 1){
                        layer.alert('删除成功');
                        $(deleteUl).remove();
